...
export default function AstrologerDetailPage() {
  ...
  const { data: astro, isLoading, isError } = useGetAstrologerQuery(astroId, {
    skip: !astroId,
  });
  const { data: reviewsData } = useGetAstrologerReviewsQuery(astroId, {
    skip: !astroId,
  });
  ...
  return (
    <div className="container py-4">
      ...
      <button
        ...
        onClick={() => navigate(-1)}
      >
        <i className="fas fa-arrow-left me-2" />Back to Astrologers
      </button>
      ...
      <div className="row g-4">
        ...
        <div className="col-lg-4">
          <div className="app-card text-center sticky-top astro-detail-card">
            ...
            {/* CTA */}
            {available ? (
              <button className="btn btn-call w-100" onClick={() => setShowBooking(true)}>
                <i className="fas fa-phone me-2" />Talk Now
              </button>
            ) : (
              <button className="btn btn-outline-secondary w-100" disabled>
                <i className="fas fa-clock me-2" />Currently Offline
              </button>
            )}
          </div>
        </div>
