...
export default function AstrologersPage() {
  ...
  // data structure: { data: Astrologer[], pagination: Pagination | null }
  const astrologers = Array.isArray(data?.data) ? data.data : [];
  const pagination  = data?.pagination ?? null;

  // Single astrologer → direct redirect
  useEffect(() => {
    if (!isLoading && !isError && astrologers.length === 1) {
      navigate(`/astrologers/${astrologers[0].id}`, { replace: true });
    }
  }, [astrologers, isLoading, isError, navigate]);
          ...
          <div className="card-body">
            <div className="row g-3 align-items-end">
              <div className="col-md-3">
                ...
                <select className="form-select form-select-sm" value={filters.expertise ?? ""} onChange={(e) => setFilter("expertise", e.target.value)}>
              ...
              <div className="col-md-3">
                ...
                <select className="form-select form-select-sm" value={filters.language ?? ""} onChange={(e) => setFilter("language", e.target.value)}>
              ...
              <div className="col-md-2">
                ...
                <select className="form-select form-select-sm" value={filters.consultation_type ?? ""} onChange={(e) => setFilter("consultation_type", e.target.value as ConsultationType)}>
              ...
              <div className="col-md-2">
                ...
                <select className="form-select form-select-sm" value={filters.min_rating ?? ""} onChange={(e) => setFilter("min_rating", e.target.value ? parseFloat(e.target.value) : undefined)}>
              ...
              <div className="col-md-2 d-flex align-items-center pb-1">
                <div className="form-check form-switch mb-0">
                  <input className="form-check-input" type="checkbox" id="onlineOnly" checked={!!filters.online} onChange={(e) => setFilter("online", e.target.checked ? true : undefined)} style={{ cursor: "pointer" }} />
                  <label className="form-check-label small fw-semibold" htmlFor="onlineOnly">Online Only</label>
                </div>
              </div>
            </div>
          </div>
              ...
              <nav>
                <ul className="pagination">
                  <li className={`page-item ${pagination.current_page === 1 ? "disabled" : ""}`}>
                    <button className="page-link" onClick={() => setFilter("page", pagination.current_page - 1)}>‹ Prev</button>
                  </li>
                  ...
                  <li className={`page-item ${pagination.current_page === pagination.last_page ? "disabled" : ""}`}>
                    <button className="page-link" onClick={() => setFilter("page", pagination.current_page + 1)}>Next ›</button>
                  </li>
                </ul>
              </nav>
            </div>
