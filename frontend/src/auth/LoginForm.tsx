...
// IMPROVE: Better UI, password show/hide, inline errors, register link
...
export default function LoginForm({ title, admin = false }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (loginThunk.rejected.match(loginRes)) {
      const msg = (loginRes.payload as any)?.message
        ?? (loginRes.payload as any)?.error
  ...
  return (
    <form onSubmit={submit}>
      ...
      <div className="mb-3">
        ...
        <input className="form-control" type="email" placeholder="rahul@email.com"
