// PATH: src/schemas/astrologer.schema.ts
import { z } from 'zod';

export const astrologerProfileSchema = z.object({
  bio:               z.string().min(20, 'Bio 20+ characters hona chahiye').max(2000),
  expertise:         z.string().min(2, 'Expertise likhein').max(200),
  experience:        z.number().min(0).max(50),
  price_per_minute:  z.number().min(1, 'Minimum ₹1/min').max(10000),
  consultation_type: z.enum(['chat', 'call', 'video', 'all']),
  languages:         z.array(z.string()).min(1, 'Kam se kam ek language select karein'),
